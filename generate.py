...
if __name__ == "__main__":
    ...
    condition = torch.tensor(flexibility_profile)
    ohaae = OHAAE()
    samples = generate(model, condition, number_of_linkers)
    for i, sample in enumerate(samples):
        ...
        rcf_10_dna = reverse_translate(deembedded_sequence)
        print(f"Sample {i+1}: AA sequence:{deembedded_sequence}, RCF10 compatible DNA sequence:{rcf_10_dna}")